...
class MultiLanguageSupport:
    ...
    def _initialize_directories(self):
        ...
        try:
            ...
            self.logger.error("Error initializing directories: %s", e)
    ...
    def _load_system_locale(self):
        ...
        try:
            ...
            self.logger.info("System locale detected: %s", system_locale)
    ...
    def _update_preferences_for_language(self, language: SupportedLanguage):
        ...
        try:
            ...
            self.logger.error("Error updating language preferences: %s", e)
    ...
    def set_language(self, language: SupportedLanguage) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info("Language successfully changed to %s", language.english_name)
                return True
            else:
                # Revert on failure
            ...
            self.logger.error("Error setting language: %s", e)
            return False
    ...
    def translate(self, key: str, category: TranslationCategory = TranslationCategory.UI_GENERAL,
        ...
        try:
            ...
            self.logger.warning("Error translating '%s': %s", key, e)
            return default or key

    def translate_plural(self, key: str, count: int, 
        ...
        try:
            ...
            self.logger.warning("Error translating plural '%s': %s", key, e)
            return f"{key} ({count})"
    ...
    def format_date(self, date_obj, format_type: str = "default") -> str:
        ...
        try:
            ...
            self.logger.error("Error formatting date: %s", e)
            return str(date_obj)
    ...
    def format_time(self, time_obj, include_seconds: bool = True) -> str:
        ...
        try:
            ...
            self.logger.error("Error formatting time: %s", e)
            return str(time_obj)
    ...
    def format_number(self, number: float, decimal_places: int = 2) -> str:
        ...
        try:
            ...
            self.logger.error("Error formatting number: %s", e)
            return str(number)
    ...
    def format_file_size(self, size_bytes: int) -> str:
        ...
        try:
            ...
            self.logger.error("Error formatting file size: %s", e)
    ...
    def export_translations_for_translation(self, output_file: str, 
                                          include_completed: bool = False) -> bool:
        """
        ...
        try:
            ...
            for category, strings in all_strings.items():
                ...
                for key, translation_data in strings.items():
                    # Include string if not completed or if include_completed is True
            ...
            self.logger.info("Translations exported to %s", output_file)
            return True
            
        except Exception as e:
            self.logger.error("Error exporting translations: %s", e)
            return False

    def import_completed_translations(self, input_file: str, 
                                    target_language: SupportedLanguage) -> bool:
        """
        ...
        try:
            ...
            for category, translations in import_data.get("translations", {}).items():
                for key, translation_data in translations.items():
                    if translation_data.get("completed", False) and translation_data.get("translation"):
                        # Store translation
                        self._store_translation(
                            target_language, category, key, 
            ...
            self.logger.info("Imported %d translations for %s", 
                           imported_count, target_language.english_name)
            return True
            
        except Exception as e:
            self.logger.error("Error importing translations: %s", e)
            return False
    ...
    def validate_translation_coverage(self, language: SupportedLanguage) -> Dict[str, Any]:
        ...
        try:
            ...
            report["coverage_percentage"] = (
                ...
                if report["total_strings"] > 0 else 0
            )
            
            return report
            
        except Exception as e:
            self.logger.error("Error validating translation coverage: %s", e)
            return {}
...
"""
            ...
            self.logger.error("Error creating PO file: %s", e)
    ...
    def _update_qt_translators(self, language: SupportedLanguage):
        ...
        try:
            ...
            self.logger.error("Error updating Qt translators: %s", e)

    def _notify_language_change(self, old_language: SupportedLanguage, 
        ...
        try:
            ...
            self.logger.info("Language changed from %s to %s", 
                           old_language.english_name, new_language.english_name)
            
        except Exception as e:
            self.logger.error("Error notifying language change: %s", e)
    ...
    def _store_translation(self, language: SupportedLanguage, category: str, 
                         key: str, translation: str):
...
def ngettext(singular: str, plural: str, count: int, 
            category: TranslationCategory = TranslationCategory.UI_GENERAL, **kwargs) -> str:
    """
...
def create_sample_translation_files():
    ...
    for language, translations in SAMPLE_TRANSLATIONS.items():
        ...
        translation_system.logger.info(
            ...
            language.english_name, len(translations)
        )
...
def demonstrate_translations():
    ...
    for language in [SupportedLanguage.ENGLISH, SupportedLanguage.SPANISH, 
        ...
        print(f"Files scanned: {_('files_scanned', TranslationCategory.SCAN_OPERATIONS, count=150)}")
