...
class BackgroundScanner:
    ...
    def __init__(self, file_scanner: Optional[ClamAVWrapper] = None):
        ...
        self.immediate_scan_extensions = {'.exe', '.dll', '.bat', '.sh', '.py', '.jar'}
    ...
    def handle_file_event(self, event: WatchEvent):
        ...
        try:
            ...
            if event.file_path not in self.active_scans:
                ...
                self.logger.debug("Queued scan for %s (priority: %s)", 
                                event.file_path, priority.name)
            
        except Exception as e:
            self.logger.error("Error handling file event for %s: %s", 
                            event.file_path, e)
    ...
    def _process_scan_task(self, task: ScanTask):
        ...
        try:
            ...
            self.logger.info("Scan completed: %s - %s (%s)", 
                           task.file_path, scan_result.result.value, 
            ...
            if task.retry_count < task.max_retries:
                ...
                self.scan_queue.put(task)
                self.logger.info("Retrying scan of %s (attempt %d/%d)", 
                               task.file_path, task.retry_count, task.max_retries)
    ...
    def _cleanup_old_results(self):
        ...
        old_results = [
            path for path, result in self.scan_results.items()
