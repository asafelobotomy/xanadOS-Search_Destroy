...
class EventProcessor:
    ...
    def process_event(self, event: WatchEvent) -> Optional[ProcessedEvent]:
        ...
        try:
            ...
            if rule.action == EventAction.ALERT and self.alert_callback:
                try:
                    self.alert_callback(event.file_path, f"Alert triggered by rule: {rule.name}")
                except Exception as e:
                    self.logger.error("Error in alert callback: %s", e)
            
            self.logger.debug("Processed event: %s -> %s (%s)", 
                            event.file_path, rule.action.value, rule.name)
    ...
    def _check_rule_conditions(self, event: WatchEvent, rule: EventRule) -> bool:
        ...
        if 'max_file_size' in rule.conditions:
            try:
                if file_path.exists() and file_path.stat().st_size > rule.conditions['max_file_size']:
                    return False
    ...
    def _extract_metadata(self, event: WatchEvent) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata.update({
                ...
                'is_executable': file_path.suffix.lower() in {'.exe', '.bat', '.sh', '.py', '.jar'},
                'is_archive': file_path.suffix.lower() in {'.zip', '.rar', '.tar', '.gz', '.7z'},
                'is_document': file_path.suffix.lower() in {'.pdf', '.doc', '.docx', '.xls', '.xlsx'},
            })
            
        except Exception as e:
            self.logger.warning("Error extracting metadata for %s: %s", event.file_path, e)
