...
from .event_processor import EventProcessor, ProcessedEvent, EventAction
...
class RealTimeMonitor:
    ...
    def _on_file_event(self, event: WatchEvent):
        ...
        try:
            ...
            if processed_event:
                ...
                self.logger.debug("File event: %s (%s) -> %s", 
                                event.file_path, 
    ...
    def _on_processed_event(self, event: ProcessedEvent):
        ...
        try:
            ...
            self.logger.error("Error executing action for %s: %s", 
                            event.original_event.file_path, e)
    ...
    def _schedule_scan(self, event: ProcessedEvent):
        ...
        self.logger.debug("Scheduled scan for %s (priority: %s)", 
                        event.original_event.file_path, priority.name)
    ...
    def _quarantine_file(self, event: ProcessedEvent):
        ...
        try:
            ...
            if file_path.exists():
                # Move to quarantine (simplified - would integrate with QuarantineManager)
                self.logger.warning("QUARANTINE: %s (rule: %s)", 
                                  file_path, event.rule_name)
                ...
                if self.file_quarantined_callback:
                    try:
                        ...
                        self.logger.error("Error in quarantine callback: %s", e)
        
        except Exception as e:
            self.logger.error("Error quarantining file %s: %s", 
                            event.original_event.file_path, e)
    ...
    def _block_file(self, event: ProcessedEvent):
        ...
        try:
            ...
            self.logger.error("Error blocking file %s: %s", 
                            event.original_event.file_path, e)
