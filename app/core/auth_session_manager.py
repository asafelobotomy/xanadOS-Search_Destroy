...
class AuthenticationSessionManager:
        ...
        with self._session_lock:
            if session_type == "global":
                if self._global_session_active and self._global_session_start:
                    ...
                    logger.debug(f"🔒 Global authentication session ended (lasted {duration:.1f} seconds)")
                self._global_session_active = False
                ...
                if session_start:
                    ...
                    logger.debug(f"🔒 Authentication session '{session_type}' ended (lasted {duration:.1f} seconds)")
    ...
    def cleanup_expired_sessions(self) -> None:
        ...
        with self._session_lock:
            ...
            if (self._global_session_active and self._global_session_start and 
                (current_time - self._global_session_start).total_seconds() >= self._session_timeout):
    ...
    def try_passwordless_sudo(self, cmd: List[str], timeout: int = 30) -> Optional[subprocess.CompletedProcess]:
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"❌ Passwordless sudo failed (rc={result.returncode}), session may have expired")
    ...
    def execute_elevated_command(self, cmd: List[str], timeout: int = 300, 
                                capture_output: bool = True, text: bool = True,
                                session_type: str = "global", operation: str = "command") -> subprocess.CompletedProcess:
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"✅ Authentication successful, session established")
    ...
    def execute_elevated_file_operation(self, operation: str, file_path: str, 
                                      content: str = None, session_type: str = "global") -> Any:
        """
    ...
    def get_session_status(self) -> Dict[str, Any]:
        ...
        with self._session_lock:
            status = {
                ...
                "global_session_start": self._global_session_start.isoformat() if self._global_session_start else None,
                "active_sessions": {
...
def execute_elevated_command(cmd: List[str], timeout: int = 300, session_type: str = "global", 
                           operation: str = "command") -> subprocess.CompletedProcess:
    """Execute elevated command with session management"""
    return auth_manager.execute_elevated_command(cmd, timeout, session_type=session_type, operation=operation)

def execute_elevated_file_operation(operation: str, file_path: str, content: str = None, 
                                  session_type: str = "global") -> Any:
