...
class DatabaseConnectionPool:
    ...
    def _create_connection(self) -> Optional[sqlite3.Connection]:
        ...
        try:
            ...
            self.logger.debug("Created database connection #%d", self.created_connections)
            return conn
...
class QueryOptimizer:
    ...
    def execute_query(self, query: str, params: tuple = (), 
                     cache_results: bool = True) -> List[sqlite3.Row]:
        """
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                ...
                if (cache_key and self._should_cache_query(query) and 
                    len(results) < 1000):  # Don't cache large result sets
                    with self.cache_lock:
...
class ScanResultsDB:
    ...
    def create_scan_session(self, scan_type: str) -> int:
        ...
        try:
            ...
            self.logger.info("Created scan session %d for type '%s'", session_id, scan_type)
            return session_id
    ...
    def add_scan_result(self, session_id: int, file_path: str, file_size: int,
                       scan_result: str, threat_name: Optional[str] = None):
        """
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                conn.execute(query, (session_id, file_path, file_size, 
    ...
    def finish_scan_session(self, session_id: int, total_files: int,
                           infected_files: int, errors: int):
        """
    ...
    def cleanup_old_sessions(self, days_to_keep: int = 30):
        ...
        if self.query_optimizer.execute_transaction(cleanup_queries):
            self.logger.info("Cleaned up scan data older than %d days", days_to_keep)
        else:
