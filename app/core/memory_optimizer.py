...
class MemoryOptimizer:
    ...
    def check_memory_pressure(self) -> bool:
        ...
        if stats.process_mb > self.memory_limit_mb:
            self.logger.warning("Process memory %.1f MB exceeds limit %d MB", 
                              stats.process_mb, self.memory_limit_mb)
            return True
        
        # Check system memory pressure
        if stats.used_percent > 90:
            self.logger.warning("System memory usage %.1f%% is critically high", 
                              stats.used_percent)
            return True
    ...
    def _clear_pools(self):
        ...
        for pool_name, pool in self.pools.items():
            with pool.lock:
                ...
                pool.pool.clear()
                self.logger.debug("Cleared pool '%s': %d objects", pool_name, cleared)
    ...
    def set_memory_warning_callback(self, callback: Callable[[MemoryStats], None]):
        """Set callback for memory warnings."""
        self.memory_warning_callback = callback
    
    def set_memory_critical_callback(self, callback: Callable[[MemoryStats], None]):
    ...
    def set(self, key, value):
        ...
        with self.lock:
            ...
            if (len(self.cache) >= self.max_size or 
                self.estimated_memory_mb + item_size_mb > self.max_memory_mb):
                self._evict_lru()
