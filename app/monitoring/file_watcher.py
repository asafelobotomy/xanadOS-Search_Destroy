...
class FileSystemWatcher:
    ...
    def __init__(self, paths_to_watch: Optional[List[str]] = None, 
            ...
            try:
                ...
                for watch_path in self.paths_to_watch:
                    ...
                    for root, dirs, files in os.walk(watch_path):
                        ...
                        # Filter directories
                        dirs[:] = [d for d in dirs if self._should_process_path(
                            os.path.join(root, d))]
                        ...
                        for file in files:
                            ...
                            try:
                                ...
                                elif (file_states[full_path]['mtime'] != stat.st_mtime or
                                      file_states[full_path]['size'] != stat.st_size):
                                    # Modified file
                                    event = WatchEvent(
                                        event_type=WatchEventType.FILE_MODIFIED,
                                        file_path=full_path,
    ...
    def _handle_event(self, event: WatchEvent):
        ...
        try:
            ...
            if (event_key in self.last_event_time and 
                current_time - self.last_event_time[event_key] < self.throttle_duration):
                return
    ...
    def _execute_event_callback(self, event: WatchEvent):
        ...
        try:
            ...
            self.logger.debug("File system event: %s - %s", 
