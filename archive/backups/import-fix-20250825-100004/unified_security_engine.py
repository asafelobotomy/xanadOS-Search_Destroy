...
class MLAnomalyDetector:
    ...
    def _detect_process_behavior_anomaly(self, event: SecurityEvent) -> float:
        ...
        try:
            ...
            recent_events = [
                e for e in self.training_data['process_behavior'][-100:]
                if e['pid'] == event.process_id and
                   event.timestamp - e['timestamp'] < 1.0
            ]
...
class SmartEventFilter:
    ...
    def __init__(self):
        ...
        self.high_risk_extensions = {
            ...
            '.appimage', '.flatpak', '.snap'  # Linux-specific
        }
        ...
        self.excluded_paths = {
            ...
            '/snap', '/var/lib/flatpak'  # Modern Linux paths
        }

        self.high_priority_paths = {
            '/home', '/usr/bin', '/usr/local/bin', '/opt', '/etc',
            '/usr/share/applications', '/var/lib'  # Critical system paths
        }
    ...
    def should_process_event(self, event: SecurityEvent) -> bool:
        ...
        try:
            ...
            if any(event.source_path.startswith(path) for path in self.high_priority_paths):
                if event.event_type in [EventType.FILE_CREATED, EventType.EXECUTABLE_MODIFIED]:
                    return True

            # Skip low-priority events for low-risk files
            if (extension not in self.medium_risk_extensions and
                event.event_type == EventType.FILE_ACCESSED):
                return False
...
class AdaptiveResourceManager:
    ...
    def analyze_system_health(self) -> SystemHealth:
        ...
        try:
            ...
            protection_mode = self._determine_optimal_mode(cpu_percent, memory.percent, disk.percent)
    ...
    def _assess_threat_level(self, cpu_usage: float, memory_usage: float, processes: List) -> ThreatLevel:
...
class UnifiedSecurityEngine:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Unified Security Engine initialized in {self.current_mode.value} mode")
            return True
...
async def demonstrate_unified_engine():
    ...
    def threat_callback(threat_info):
        print(f"🚨 THREAT: {threat_info['event'].source_path} "
              f"(score: {threat_info['anomaly_score']:.3f})")

    def performance_callback(health: SystemHealth):
        print(f"📊 Health - CPU: {health.cpu_usage:.1f}%, "
    ...
    try:
        ...
        if await engine.initialize():
            ...
            if await engine.start_protection():
                ...
                print(f"   Avg processing time: {status['performance_metrics']['avg_processing_time']:.2f}ms")
