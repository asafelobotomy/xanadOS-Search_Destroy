...
def build_general_page(host):
    ...
    if not hasattr(host, "settings_minimize_to_tray_cb"):
        host.settings_minimize_to_tray_cb = QCheckBox("Minimize to System Tray")
        host.settings_minimize_to_tray_cb.setChecked(True)
    if not hasattr(host, "settings_show_notifications_cb"):
        host.settings_show_notifications_cb = QCheckBox("Show Notifications")
        host.settings_show_notifications_cb.setChecked(True)
    form = QFormLayout()
    form.addRow("Activity Log Retention:", host.settings_activity_retention_combo)
    form.addRow(host.settings_minimize_to_tray_cb)
    form.addRow(host.settings_show_notifications_cb)
...
def build_scanning_page(host):
    ...
    if not hasattr(host, "settings_scan_archives_cb"):
        host.settings_scan_archives_cb = QCheckBox("Scan Archive Files")
        host.settings_scan_archives_cb.setChecked(True)
    if not hasattr(host, "settings_follow_symlinks_cb"):
        host.settings_follow_symlinks_cb = QCheckBox("Follow Symbolic Links")
        host.settings_follow_symlinks_cb.setChecked(False)
    ...
    if not hasattr(host, "exclusion_text"):
        host.exclusion_text = QTextEdit()
        host.exclusion_text.setMaximumHeight(60)
    form = QFormLayout()
    form.addRow("Max Threads:", host.settings_max_threads_spin)
    form.addRow("Scan Timeout:", host.settings_timeout_spin)
    form.addRow(host.settings_scan_archives_cb)
    form.addRow(host.settings_follow_symlinks_cb)
    form.addRow("Scan Depth:", host.scan_depth_combo)
    form.addRow("File Types:", host.file_filter_combo)
    form.addRow("Memory Limit:", host.memory_limit_combo)
    form.addRow("Exclusion Patterns:", host.exclusion_text)
    layout.addLayout(form)
    layout.addStretch()
    return page
...
def build_scheduling_page(host):
    ...
    if not hasattr(host, "settings_scan_time_edit"):
        host.settings_scan_time_edit = host._make_time_edit()
        host.settings_scan_time_edit.setTime(QTime(2, 0))
    ...
    form.addRow(host.settings_enable_scheduled_cb)
    form.addRow("Scan Frequency:", host.settings_scan_frequency_combo)
    form.addRow("Scan Time:", host.settings_scan_time_edit)
    form.addRow("Scan Type:", host.settings_scan_type_combo)
    form.addRow("Custom Directory:", host.settings_custom_dir_widget)
...
def build_security_page(host):
    ...
    if not hasattr(host, "settings_auto_update_cb"):
        host.settings_auto_update_cb = QCheckBox("Auto-update Virus Definitions")
        host.settings_auto_update_cb.setChecked(True)
    layout.addWidget(host.settings_auto_update_cb)
    layout.addStretch()
    return page
...
def build_firewall_page(host):
    ...
    if not hasattr(host, "settings_firewall_auto_detect_cb"):
        host.settings_firewall_auto_detect_cb = QCheckBox(
            "Enable automatic firewall detection and monitoring"
        )
        host.settings_firewall_auto_detect_cb.setChecked(True)
        host.settings_firewall_auto_detect_cb.setToolTip(
            "Automatically detect and monitor your system's firewall status"
        )
    status_layout.addWidget(host.settings_firewall_auto_detect_cb)

    # Notification settings
    if not hasattr(host, "settings_firewall_notify_changes_cb"):
        host.settings_firewall_notify_changes_cb = QCheckBox(
            "Notify when firewall status changes externally"
        )
        host.settings_firewall_notify_changes_cb.setChecked(True)
        host.settings_firewall_notify_changes_cb.setToolTip(
            "Show notifications when firewall is enabled/disabled outside the application"
        )
    ...
    if not hasattr(host, "settings_preferred_firewall_combo"):
        host.settings_preferred_firewall_combo = (
    ...
    if not hasattr(host, "settings_firewall_auto_load_modules_cb"):
        host.settings_firewall_auto_load_modules_cb = QCheckBox(
            "Attempt to load missing kernel modules"
        )
        host.settings_firewall_auto_load_modules_cb.setChecked(True)
        host.settings_firewall_auto_load_modules_cb.setToolTip(
            "Try to load iptables kernel modules if they're missing"
        )
    ...
    if not hasattr(host, "settings_firewall_debug_logging_cb"):
        ...
        host.settings_firewall_debug_logging_cb.setToolTip(
            "Log detailed firewall operation information for troubleshooting"
        )
...
def build_rkhunter_page(host):
    ...
    if not hasattr(host, "settings_rkhunter_test_categories"):
        host.settings_rkhunter_test_categories = {
            "rootkits": {
                ...
                "description": "Known rootkits signatures",
                "default": True,
                "priority": 1,
            },
            "system_commands": {
                "name": "System Commands",
                "description": "System command integrity",
                "default": True,
                "priority": 1,
            },
            "network": {
                "name": "Network Security",
                "description": "Interfaces and ports",
                "default": True,
                "priority": 2,
            },
            "system_integrity": {
                "name": "System Integrity",
                "description": "Filesystem & config verification",
                "default": True,
                "priority": 2,
            },
            "applications": {
                "name": "Applications",
                "description": "Hidden processes & files",
                "default": False,
                "priority": 3,
            },
        }
    ...
    sorted_categories = sorted(
        host.settings_rkhunter_test_categories.items(),
    ...
    if not hasattr(host, "rkhunter_update_mirrors_btn"):
        ...
        host.rkhunter_update_mirrors_btn.clicked.connect(
            lambda: host.run_rkhunter_optimization("update_mirrors")
        )

    if not hasattr(host, "rkhunter_update_baseline_btn"):
        host.rkhunter_update_baseline_btn = QPushButton("Update Baseline")
        host.rkhunter_update_baseline_btn.clicked.connect(
            lambda: host.run_rkhunter_optimization("update_baseline")
        )

    if not hasattr(host, "rkhunter_optimize_config_btn"):
        host.rkhunter_optimize_config_btn = QPushButton("Optimize Configuration")
        host.rkhunter_optimize_config_btn.clicked.connect(
            lambda: host.run_rkhunter_optimization("optimize_config")
        )
...
def build_updates_page(host):
    ...
    if not hasattr(host, "settings_update_check_interval_spin"):
        host.settings_update_check_interval_spin = (
            host.NoWheelSpinBox() if hasattr(host, "NoWheelSpinBox") else QSpinBox()
        )
    ...
    if not hasattr(host, "update_status_label"):
        host.update_status_label = QLabel(
            "Click 'Check for Updates Now' to check for the latest version"
        )
