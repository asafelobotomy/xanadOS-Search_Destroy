...
class RKHunterScanDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for category_id, category_info in test_categories.items():
            ...
            desc_label.setStyleSheet(f"color: {self.get_theme_color('secondary_text') if hasattr(self, 'get_theme_color') else '#666'}; font-size: 11px; margin-left: 20px; margin-bottom: 10px;")
            desc_label.setWordWrap(True)
        ...
        start_btn.setStyleSheet(
            f"QPushButton {{ background-color: {self.get_theme_color('success') if hasattr(self, 'get_theme_color') else '#007bff'}; "
            f"color: {self.get_theme_color('primary_text') if hasattr(self, 'get_theme_color') else 'white'}; "
    ...
    def run(self):
        ...
        try:
            ...
            if not self.rkhunter.is_functional():
                if pkexec_available:
                    self.progress_updated.emit("🔐 GUI password dialog will appear for authentication...")
                else:
                    self.progress_updated.emit("⚠️ Terminal password prompt may appear...")
            ...
            if pkexec_available:
                self.progress_updated.emit("Updating RKHunter database (GUI authentication)...")
            else:
                self.progress_updated.emit("Updating RKHunter database (may require password)...")
                
            update_success = self.rkhunter.update_database()
            
            if not update_success:
                self.logger.warning("Failed to update RKHunter database, continuing with scan")
                self.progress_updated.emit("Database update failed, continuing with existing database...")
            else:
