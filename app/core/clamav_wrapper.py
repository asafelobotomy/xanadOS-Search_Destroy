...
# Privilege escalation helpers (optional import)
try:
    from app.core.elevated_runner import elevated_run, validate_auth_session
except ImportError:  # elevated runner not available in some builds
    elevated_run = None  # type: ignore

    def validate_auth_session() -> bool:  # type: ignore
        return False
...
class ClamAVWrapper:
    ...
    def _check_clamav_availability(self) -> bool:
        ...
        try:
            # Build command only when path is available
            cmd = [self.clamscan_path, "--version"]
            result = run_secure(cmd, timeout=10, capture_output=True, text=True)
        ...
        try:
            ...
            if p.is_symlink():
                return ScanFileResult(
                    ...
                    error_message="Symlink not allowed",
                )
            if os.name == "posix":
                st = p.stat()
                if st.st_mode & 0o002:
                    return ScanFileResult(
                        file_path=file_path,
                        result=ScanResult.ERROR,
                        error_message="World-writable file blocked",
                    )
        except OSError:
            return ScanFileResult(
                file_path=file_path,
        ...
        if use_daemon and self.clamdscan_path and not os.environ.get("FLATPAK_ID"):
            ...
            if self._is_clamd_running():
                try:
                    ...
                    self.logger.warning("Daemon scan failed, falling back to clamscan: %s", e)
            ...
            if d.is_symlink():
                return [
                    ScanFileResult(
                        ...
                        result=ScanResult.ERROR,
                        error_message="Symlink directory not allowed",
                    )
                ]
            if os.name == "posix":
                st = d.stat()
                if st.st_mode & 0o002:
                    return [
                        ScanFileResult(
                            file_path=directory_path,
                            result=ScanResult.ERROR,
                            error_message="World-writable directory blocked",
                        )
                    ]
        except OSError:
            return [
    ...
    def _build_scan_options(self, quick_scan: bool = False, **kwargs) -> list[str]:
        ...
        max_scan_size = performance_settings.get(
            "max_scan_size", "200M" if not quick_scan else "50M"
        )
        options.extend(["--max-scansize", max_scan_size])

        # Custom database path or bundled db inside Flatpak
        if self.custom_db_path.exists() and any(self.custom_db_path.iterdir()):
            ...
            if bundled_db.exists():
                options.extend(["--database", str(bundled_db)])
    ...
    def get_version_info(self) -> dict[str, str]:
        ...
        # Get database info
        try:
            db_cmd = [self.clamscan_path, "--database-info"]
            if self.custom_db_path.exists():
                db_cmd.extend(["--database", str(self.custom_db_path)])
            else:
                bundled_db = Path("/app/share/clamav")
                if bundled_db.exists():
                    db_cmd.extend(["--database", str(bundled_db)])
            result = run_secure(db_cmd, timeout=10, capture_output=True, text=True, check=False)
            if result.returncode == 0:
                lines = result.stdout.strip().split("\n")
                for line in lines:
    ...
    def get_security_recommendations(self) -> list[str]:
        ...
        if "ClamAV 1.4" not in version_info.get("engine_version", ""):
            recommendations.append(
                "Update to ClamAV 1.4 LTS for latest security fixes and performance improvements"
            )
        ...
        # Check daemon configuration (skip inside Flatpak)
        if (
            self.clamdscan_path
            and not os.environ.get("FLATPAK_ID")
            and not self._is_clamd_running()
        ):
            recommendations.append(
                "Start ClamAV daemon (clamd) for better performance and security"
            )
    ...
    def optimize_for_parallel_scanning(self, max_threads: int = None) -> dict[str, Any]:
        ...
        if max_threads is None:
            try:
                import psutil  # local import to avoid hard dependency
    ...
    def should_scan_file(self, file_path: str, quick_scan: bool = False) -> bool:
        ...
        try:
            ...
            if quick_scan:
                max_quick_size = self.config.get("performance", {}).get(
                    "quick_scan_max_file_size", 50 * 1024 * 1024
                )  # 50MB
            ...
            if file_extension in media_extensions:
                max_media_size = self.config.get("performance", {}).get(
        ...
        if config_path.exists():
            try:
                with open(config_path, encoding="utf-8") as f:
                    config = f.read()
                    if any(
                        param in config for param in ["MaxScanSize", "MaxFileSize", "MaxRecursion"]
                    ):
    ...
    def implement_multithreaded_scanning(
        ...
        if self._is_clamd_running():
            self.logger.info("Using daemon-based parallel scanning with %d workers", max_workers)
    ...
    def update_virus_definitions(self) -> bool:
        ...
        update_commands = [
            ...
            [self.freshclam_path, "--verbose", "--datadir", str(self.custom_db_path)],
        ]
        ...
        for i, cmd in enumerate(update_commands):
            try:
                self.logger.debug("Attempt %d: Running command: %s", i + 1, " ".join(cmd))

                # Skip privileged commands if we're already running as root
                if os.getuid() == 0 and i >= 2:  # Skip elevated attempts if root
                    self.logger.debug("Already running as root, skipping elevated command")
                    continue
                ...
                if i == len(update_commands) - 1:
                    if elevated_run is None:
                        self.logger.warning(
                            "Elevated runner not available; skipping GUI authentication path"
                        )
                        continue

                    # Hint if no valid auth session is present
                    if not validate_auth_session():
                        self.logger.debug(
                            "No cached auth session; GUI escalation may prompt for credentials"
                        )
                    self.logger.info(
                        "Using elevated privileges with GUI authentication (same as RKHunter)"
                    )
                    result = elevated_run(
                        cmd,
                        capture_output=True,
                    ...
                    if (
                        ...
                        self.logger.info("Virus definitions already up to date")
                        return True

                    self.logger.warning(
                        "Elevated update failed (code %s): %s",
                        result.returncode,
                        result.stderr,
                    )
                    continue
                    ...
                    if (
                        ...
                        self.logger.warning(
                            "Permission error with attempt %d, trying next method...",
                            i + 1,
                        )
                        continue
                    ...
                    if i < len(update_commands) - 1:
                        self.logger.warning("Update attempt %d failed, trying next method", i + 1)

            except subprocess.TimeoutExpired:
                self.logger.error(
                    "Virus definition update attempt %d timed out after 5 minutes",
                    i + 1,
                )
                continue
            except FileNotFoundError as e:
                self.logger.error("Command not found in attempt %d: %s", i + 1, e)
                continue
            except (subprocess.SubprocessError, OSError, ValueError) as e:
                self.logger.error("Error in update attempt %d: %s", i + 1, str(e))
                continue
        ...
        self.logger.error("1. You have write permissions to /var/lib/clamav/, or")
        self.logger.error("2. GUI authentication is working properly, or")
        self.logger.error("3. Set up a custom database directory with proper permissions")
        return False
    ...
    def _update_custom_source(self, source_url: str) -> bool:
        ...
        try:
            ...
            filename = hashlib.md5(source_url.encode(), usedforsecurity=False).hexdigest() + ".cvd"
    ...
    def check_definition_freshness(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("Error checking definition freshness: %s", e)
            info["error"] = str(e)
            info["needs_update"] = True
