...
class ClamAVWrapper:
    ...
    def __init__(self):
        try:
            from utils.config import setup_logging, load_config, CACHE_DIR
        except ImportError:
    ...
    def scan_file(self, file_path: str, **kwargs) -> ScanFileResult:
        ...
        try:
            ...
            return self._parse_scan_output(
                ...
                result.returncode, file_size, scan_time
            )
    ...
    def scan_directory(self, directory_path: str, **kwargs) -> List[ScanFileResult]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=self.config['advanced_settings']['scan_timeout'] * 10  # Longer timeout for directories
            )
            
            # Parse directory scan results
            return self._parse_directory_scan_output(result.stdout, result.stderr, result.returncode)
    ...
    def _build_scan_options(self, **kwargs) -> List[str]:
        ...
        # Executable scanning
        if kwargs.get('scan_executable', scan_settings.get('scan_executable', True)):
    ...
    def _parse_scan_output(self, file_path: str, stdout: str, stderr: str, 
                          returncode: int, file_size: int, scan_time: float) -> ScanFileResult:
    ...
    def _parse_directory_scan_output(self, stdout: str, stderr: str, returncode: int) -> List[ScanFileResult]:
    ...
    def update_virus_definitions(self) -> bool:
        ...
        update_commands = [
            # First try without sudo (in case user has permissions or custom database directory)
        ...
        if pkexec_path:
            ...
            update_commands.append(['pkexec', self.freshclam_path, '--verbose'])
        ...
        for i, cmd in enumerate(update_commands):
            try:
                self.logger.debug(f"Attempt {i+1}: Running command: {' '.join(cmd)}")
                
                # Skip sudo command if we're already running as root
                if cmd[0] == 'sudo' and os.getuid() == 0:
                    self.logger.debug("Already running as root, skipping sudo command")
                    continue
                
                # Special handling for privilege escalation commands
                if cmd[0] in ['sudo', 'pkexec']:
                    if cmd[0] == 'pkexec':
                        self.logger.info("Using pkexec for GUI password prompt")
                        ...
                        if result.returncode == 0:
                            self.logger.info("Virus definitions updated successfully with pkexec")
                            return True
                            
                        # Check if definitions are already up to date
                        output_text = result.stdout + result.stderr
                        if "is up to date" in output_text.lower() or "database is up-to-date" in output_text.lower():
                            self.logger.info("Virus definitions already up to date")
                            return True
                            
                        self.logger.warning(f"pkexec update attempt {i+1} failed (code {result.returncode}): {result.stderr}")
                        continue
                        
                    else:  # sudo
                        self.logger.info("Running with sudo - you may be prompted for your password")
                        # Run sudo command without capturing output so user can interact
                        result = subprocess.run(
                            cmd,
                            text=True,
                            timeout=300,
                        ...
                        if result.returncode == 0:
                            self.logger.info("Virus definitions updated successfully with sudo")
                            return True
                        else:
                            self.logger.warning(f"Sudo update attempt {i+1} failed (code {result.returncode})")
                            continue
                else:
                    ...
                    if "permission denied" in result.stderr.lower() or "can't create temporary directory" in result.stderr.lower():
                        self.logger.warning(f"Permission error with attempt {i+1}, trying next method...")
                        continue
                        
                    # If this was the last attempt, log the failure
                    if i == len(update_commands) - 1:
                        self.logger.error(f"Failed to update virus definitions (code {result.returncode}): {result.stderr}")
                
            except subprocess.TimeoutExpired:
                self.logger.error(f"Virus definition update attempt {i+1} timed out after 5 minutes")
                continue
            except FileNotFoundError as e:
                if 'sudo' in str(e) and cmd[0] == 'sudo':
                    self.logger.warning("sudo not available, skipping sudo method")
                    continue
                else:
                    self.logger.error(f"Command not found in attempt {i+1}: {e}")
                    continue
