...
class FirewallDetector:
        ...
        if self._has_systemd():
            try:
                result = subprocess.run(
                    ['systemctl', 'list-unit-files', f"{fw_info['service_name']}.service"],
                    capture_output=True,
                    text=True,
                    timeout=5,
                    check=False
    ...
    def _get_ufw_status(self) -> Dict[str, str | bool]:
        ...
        try:
            # First try without sudo (some systems allow status check without root)
            result = subprocess.run(
                ['ufw', 'status'],
                capture_output=True,
                text=True,
    ...
    def _get_ufw_service_status(self) -> Dict[str, str | bool]:
        ...
        try:
            ...
            # Method 2: Try to check if UFW status file exists and indicates enabled
            ufw_status_paths = [
    ...
    def _get_iptables_status(self) -> Dict[str, str | bool]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                rule_count = len([line for line in lines if line.strip() and not line.startswith('Chain') and not line.startswith('target')])
    ...
    def _toggle_ufw(self, enable: bool) -> Dict[str, str | bool]:
        ...
        try:
            ...
            cmd = admin_cmd + ['ufw', '--force', 'enable' if enable else 'disable']
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                ...
                if 'request dismissed' in error_output.lower() or 'cancelled' in error_output.lower():
                    return {
                        'success': False,
    ...
    def _toggle_iptables(self, enable: bool) -> Dict[str, str | bool]:
        ...
        try:
            ...
            if enable:
                ...
                commands = [
                    admin_cmd + ['iptables', '-P', 'INPUT', 'DROP'],
                    admin_cmd + ['iptables', '-P', 'FORWARD', 'DROP'],
                    admin_cmd + ['iptables', '-P', 'OUTPUT', 'ACCEPT'],
                    admin_cmd + ['iptables', '-A', 'INPUT', '-i', 'lo', '-j', 'ACCEPT'],
                    admin_cmd + ['iptables', '-A', 'INPUT', '-m', 'conntrack', '--ctstate', 'ESTABLISHED,RELATED', '-j', 'ACCEPT']
                ]
            else:
                ...
                commands = [
                    ...
                    admin_cmd + ['iptables', '-X']   # Delete user-defined chains
                ]
