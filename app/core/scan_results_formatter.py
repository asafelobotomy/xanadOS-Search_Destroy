...
class ModernScanResultsFormatter:
    ...
    def format_rkhunter_output(self, raw_output: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(skip_phrase in line.lower() for skip_phrase in [
                'grep: warning:', 'egrep: warning:', 'unknown option'
            ]):
                continue
            ...
            if line.startswith('Checking ') or 'Performing' in line:
                ...
                if current_section and section_items:
                    formatted_lines.extend(self._format_check_section(current_section, section_items, stats))
                    section_items = []
            ...
            if any(marker in line for marker in ['[ OK ]', '[ Not found ]', '[ Warning ]', '[ Infected ]', '[ Skipped ]']):
    ...
    def _format_check_section(self, section_name: str, items: List[tuple], stats: Dict) -> List[str]:
        ...
        if grouped[ScanResultType.WARNING]:
            ...
            if len(grouped[ScanResultType.WARNING]) > 5:
                section_lines.append(f"   ... and {len(grouped[ScanResultType.WARNING])-5} more warnings")
            section_lines.append("")
    ...
    def format_scan_completion(self, scan_type: str, duration: float, 
                             files_scanned: int = 0, threats_found: int = 0) -> List[str]:
    ...
    def _parse_check_result(self, line: str) -> tuple:
        ...
        # Clean up the text
        cleaned_text = line
        for marker in ['[ OK ]', '[ Not found ]', '[ None found ]', '[ Warning ]', '[ Infected ]', '[ Skipped ]']:
    ...
    def format_combined_scan_summary(self, rkhunter_results: Dict, 
                                   clamav_results: Dict) -> List[str]:
