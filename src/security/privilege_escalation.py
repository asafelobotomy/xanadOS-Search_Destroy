...
class PrivilegeEscalationManager:
    ...
    def __init__(self):
        ...
        self._policy_file = Path(__file__).parent.parent / "config" / "org.xanados.searchanddestroy.policy"
    ...
    def request_elevation(self, request: ElevationRequest) -> Tuple[bool, str, str]:
        ...
        """
        self.logger.info("Requesting elevation for operation: %s", request.operation.value)
        ...
        try:
            ...
            if self._polkit_available:
                ...
                self.logger.warning("Using sudo fallback for privilege escalation")
            ...
            if success:
                self.logger.info("Elevation successful for operation: %s", request.operation.value)
            else:
                self.logger.error("Elevation failed for operation: %s, return code: %d", 
                                request.operation.value, result.returncode)
            ...
            error_msg = f"Elevation failed for operation {request.operation.value}: {e}"
            self.logger.error(error_msg)
            raise SecureElevationError(error_msg)
    ...
    def scan_system_directories(self, scan_paths: List[str], scan_options: Optional[Dict[str, str]] = None) -> Tuple[bool, str, str]:
        """
        ...
        # Build scan command
        scan_command = [
            sys.executable, "-m", "app.core.file_scanner",
            "--scan-paths", *validated_paths
        ]
    ...
    def manage_quarantine(self, action: str, file_path: str) -> Tuple[bool, str, str]:
        ...
        request = ElevationRequest(
            ...
            command=[
                sys.executable, "-m", "app.core.quarantine_manager",
                f"--{action}", file_path
            ],
            working_directory=str(Path(__file__).parent.parent),
    ...
    def install_policy_file(self) -> bool:
        ...
        policy_dest = Path("/usr/share/polkit-1/actions/org.xanados.searchanddestroy.policy")
        ...
        try:
            ...
            subprocess.run([
                "sudo", "chmod", "644", str(policy_dest)
