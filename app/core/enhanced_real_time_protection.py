...
class AdaptiveResourceManager:
    ...
    def get_current_metrics(self) -> SystemResourceMetrics:
        ...
        try:
            ...
            if hasattr(self, "_last_disk_io") and hasattr(self, "_last_net_io"):
                ...
                disk_io_rate = (
                    (disk_io.read_bytes + disk_io.write_bytes - self._last_disk_io)
                    / time_delta
                    / (1024 * 1024)
                )  # MB/s
                network_io_rate = (
                    (network_io.bytes_sent + network_io.bytes_recv - self._last_net_io)
                    / time_delta
                    / (1024 * 1024)
                )  # MB/s
            else:
    ...
    def get_optimal_mode(self) -> ProtectionMode:
        ...
        if (
            avg_cpu > self.performance_thresholds["cpu_high"]
            or avg_memory > self.performance_thresholds["memory_high"]
        ):
            return ProtectionMode.PERFORMANCE

        # Low resource usage - can use maximum protection
        elif (
            avg_cpu < self.performance_thresholds["cpu_low"]
            and avg_memory < self.performance_thresholds["memory_low"]
        ):
    ...
    async def _adaptive_monitoring_loop(self):
        ...
        while self.is_active:
            try:
                ...
                self.resource_manager.get_current_metrics()
    ...
    async def _threat_processing_loop(self):
        ...
        while self.is_active:
            try:
                ...
                self.resource_manager.get_scan_parameters(
...
async def performance_benchmark():
    ...
    for test_file in test_files:
        ...
        total_processed += 1
        print(
            f"✅ {Path(test_file).name}: {result.threat_level.description} "
