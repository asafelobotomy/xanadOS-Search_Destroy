...
FORBIDDEN_PATHS = [
    '/proc', '/sys', '/dev', '/run', '/tmp/systemd-private-*',
    '/etc/shadow', '/etc/passwd', '/etc/sudoers',
    '/boot', '/lost+found', '/var/run', '/var/lock'
]

# Dangerous file extensions that need special handling
DANGEROUS_EXTENSIONS = {
    '.exe', '.scr', '.bat', '.cmd', '.com', '.pif', '.vbs', '.js',
    '.jar', '.app', '.deb', '.rpm', '.pkg', '.dmg', '.iso'
}
            ...
            if path_obj.is_dir():
                ...
                try:
                    ...
                    if file_count > MAX_FILES_PER_SCAN:
                        return False, f"Directory contains too many files: {file_count} > {MAX_FILES_PER_SCAN}"
                except (OSError, PermissionError):
                    # If we can't count files, it might be too large or inaccessible
            ...
            try:
                ...
                if self._is_archive(file_obj):
                    if file_size > MAX_ARCHIVE_SIZE:
                        return False, f"Archive too large: {file_size} bytes > {MAX_ARCHIVE_SIZE}"
...
def validate_scan_request(scan_path: str, max_depth: Optional[int] = None) -> dict:
    """
