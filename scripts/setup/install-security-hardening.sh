...
if [[ -f "$POLICY_DIR/org.xanados.searchanddestroy.policy" ]]; then
    sudo cp "$POLICY_DIR/org.xanados.searchanddestroy.policy" \
         "$POLICY_DIR/org.xanados.searchanddestroy.policy.backup-$(date +%Y%m%d-%H%M%S)"
    echo "   ✅ Backed up main policy"
fi

if [[ -f "$POLICY_DIR/org.xanados.searchanddestroy.rkhunter.policy" ]]; then
    sudo cp "$POLICY_DIR/org.xanados.searchanddestroy.rkhunter.policy" \
         "$POLICY_DIR/org.xanados.searchanddestroy.rkhunter.policy.backup-$(date +%Y%m%d-%H%M%S)"
    echo "   ✅ Backed up RKHunter policy"
fi

# Install hardened policy
echo "🛡️ Installing hardened PolicyKit configuration..."
if [[ -f "$CONFIG_DIR/org.xanados.searchanddestroy.hardened.policy" ]]; then
    sudo cp "$CONFIG_DIR/org.xanados.searchanddestroy.hardened.policy" \
         "$POLICY_DIR/org.xanados.searchanddestroy.hardened.policy"
    echo "   ✅ Installed hardened policy"
    
    # Set proper permissions
    sudo chmod 644 "$POLICY_DIR/org.xanados.searchanddestroy.hardened.policy"
    sudo chown root:root "$POLICY_DIR/org.xanados.searchanddestroy.hardened.policy"
    echo "   ✅ Set secure permissions"
else
    echo "   ❌ Hardened policy file not found: $CONFIG_DIR/org.xanados.searchanddestroy.hardened.policy"
    exit 1
fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    if [[ -f "$POLICY_DIR/org.xanados.searchanddestroy.policy" ]]; then
        sudo rm "$POLICY_DIR/org.xanados.searchanddestroy.policy"
        echo "   ✅ Removed old main policy"
    fi
    
    if [[ -f "$POLICY_DIR/org.xanados.searchanddestroy.rkhunter.policy" ]]; then
        sudo rm "$POLICY_DIR/org.xanados.searchanddestroy.rkhunter.policy"
...
if sudo pkaction --action-id org.xanados.searchanddestroy.rkhunter.scan &> /dev/null; then
